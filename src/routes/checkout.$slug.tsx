...
import { formatPrice, getProduct, toPersianDigits } from "@/lib/products";
...
export const Route = createFileRoute("/checkout/$slug")({
  ...
  head: ({ loaderData }) => {
    if (!loaderData) {
      return { meta: [{ title: "پرداخت کارت به کارت | VAICH" }, { name: "robots", content: "noindex" }] };
    }
    const title = `پرداخت اشتراک ${loaderData.product.name} | VAICH`;
    return {
      meta: [
        { title },
        { name: "description", content: `پرداخت کارت به کارت اشتراک ${loaderData.product.name} در VAICH.` },
        { property: "og:title", content: title },
        { property: "og:description", content: `پرداخت کارت به کارت اشتراک ${loaderData.product.name}.` },
        ...
        <div className="space-y-6">
          ...
          <div className="glass-panel rounded-3xl border border-border p-6">
            ...
            <div className="rounded-2xl border border-neon-blue/40 bg-background/60 p-5">
              ...
              <p dir="ltr" className="mt-2 select-all text-center text-xl font-extrabold tracking-[0.2em] sm:text-2xl">
                {CARD_NUMBER_GROUPED}
              </p>
              <p className="mt-2 text-center text-sm text-muted-foreground" dir="rtl">
                {toPersianDigits(CARD_NUMBER_GROUPED)}
              </p>
          ...
          {showUpload ? (
            <div className="rounded-3xl border border-border bg-card/70 p-6">
              <h2 className="mb-1 text-base font-bold">۳. بارگذاری رسید پرداخت</h2>
              ...
              {fileError ? (
                <p role="alert" className="mt-4 rounded-2xl border border-destructive/50 bg-background/60 p-4 text-sm">
                  {fileError}
                </p>
              ) : null}
              ...
              {preview ? (
                <div className="mt-5">
                  ...
                  <div className="mt-5 rounded-2xl border border-border bg-background/50 p-4 text-xs leading-7 text-muted-foreground">
                    سایت به‌صورت خودکار فایل را به تلگرام ارسال نمی‌کند. با زدن دکمهٔ زیر گفت‌وگوی
                    تلگرام با <span dir="ltr">@{CONTACT_TELEGRAM_ID}</span> باز می‌شود؛ همان تصویر
                    رسید را در آن گفت‌وگو بفرستید و نام محصول ({product.name}) و مبلغ (
                    {formatPrice(product.price)}) را بنویسید.
                  </div>
                  ...
                  <a
                    ...
                    className={`${neonButtonClass("primary", "lg")} mt-4 w-full`}
                  >
        ...
        <aside className="h-fit space-y-4 rounded-3xl border border-border bg-card/70 p-6 lg:sticky lg:top-24">
          ...
          <dl className="space-y-3 text-sm">
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <dd className="font-medium" dir="ltr">{CARD_NUMBER_GROUPED}</dd>
            </div>
            <div className="flex items-center justify-between gap-4">
